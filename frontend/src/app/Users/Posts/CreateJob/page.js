...
const CreateJob = () => {
    ...
    useEffect(() => {
        const fetchRecruiterData = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:3001/get_recruiter_company", { withCredentials: true });
                const response1 = await axios.get("http://127.0.0.1:3001/has-ai-subscription", { withCredentials: true });
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate that interview_type is selected before submitting

        const finalFormData = {
            ...formData,
            company_name: formData?.new_company_name, // Include company name
        };

        try {
            await axios.post("http://127.0.0.1:3001/createjob/", finalFormData, { withCredentials: true });
