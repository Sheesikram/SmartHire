    ...
    return (
        <div className="min-h-screen  text-gray-900 font-sans py-12 mt-12">
            {showPracticeModule ? (
                ...
                // Subscription Plans
                <>
                    <h1 className="text-center text-5xl md:text-6xl font-extrabold text-gradient bg-[#F4F2EE] text-[#0073b1] bg-clip-text mb-16">
                        Choose Your Plan
                    </h1>

                    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 px-8">
                        {/* Basic Plan */}
                        <div className={`bg-white rounded-3xl shadow-md p-8 text-center border ${hasSubscription ? 'bg-gray-100' : 'border-gray-300'} transition-all transform hover:scale-100 hover:shadow-none`}>
                            <h2 className="text-3xl font-bold text-gray-800 mb-6">Basic</h2>
                            <p className="text-5xl font-extrabold text-gray-900 mb-4">$0.00</p>
                            <p className="text-lg text-gray-500 mb-8">Billed monthly</p>
                            ...
                            <button
                                className="mt-10 px-8 py-4 bg-gray-800 text-white font-semibold text-xl rounded-xl transition-all hover:bg-gray-700"
                                disabled={hasSubscription}
                                onClick={handlePayment}>
                                {hasSubscription ? "Unlocked" : "Already Selected"}
                            </button>
                        </div>

                        {/* Subscription Plan */}
                        <div className={`bg-white rounded-3xl shadow-md p-8 text-center border border-gray-200 transition-all transform hover:scale-100 hover:shadow-none relative overflow-hidden`}>
                            <div className="absolute top-0 left-0 w-full h-3 bg-gradient-to-r from-green-500 to-green-700" />
                            <h2 className="text-3xl font-bold text-gray-800 mb-6">Subscription</h2>
                            <p className="text-5xl font-extrabold text-gray-900 mb-4">$49.99</p>
                            <p className="text-lg text-gray-500 mb-8">Billed monthly</p>
                            <ul className="text-left text-gray-700 space-y-6">
                                <li className="flex items-center gap-4">
                            ...
                            <button
                                className="mt-10 px-8 py-4 bg-gradient-to-r from-green-500 to-green-700 text-white font-semibold text-xl rounded-xl shadow-lg transition-all hover:bg-green-600"
                                disabled={isLoading || hasSubscription}
                                onClick={handlePayment}>
                                {hasSubscription ? "Unlocked" : "Select Plan"}
                            </button>
                        </div>
                    </div>
                </>
